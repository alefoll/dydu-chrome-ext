...
//         request.sent = JSON.parse(parameters.data);

//         if (har_entry.response.status >= 200 && har_entry.response.status < 300 && /^dydu\.(.*)\.callbacks\._([0-9]*)\(/.test(har_entry.response.content))
//             request.response = JSON.parse(har_entry.response.content.replace(/^dydu\.(.*)\.callbacks\._([0-9]*)\(/, "").slice(0, -1));
...
chrome.devtools.panels.create('DYDU', null, 'panel.html', (extensionPanel) => {
    ...
    extensionPanel.onShown.addListener(function temp(panelWindow) {
        ...
        while (message = history.shift())
            _window.requestHandler(message);
    });
});
